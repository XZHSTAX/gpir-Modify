...
今天主要在调参，包括但不限于速度PID的调参以及权限分配参数的调参，本次提交内容包括：

1. 对采集到的力矩使用低通滤波器，滤除高频噪声，方便后续使用
2. 对权限分配中的S进行滤波，并调整对应的阈值参数s1
3. 为权限分配添加了一个新的消息类型，方便查看内部参数Q,QT,S,alpha

现在发现的问题，原有的权限分配存在以下问题：

1. 设计不合理，比如从状态3回到状态1，驾驶员的权限会出现下降，这在实验中可以看到，后面要考虑更新权限分配的设计，或者其他方式把这一点规避

## 250730

今天还是在调参，调整了一些参数，并且设置了新的纯追踪控制器，来实现车辆的横向控制，这样绘图更好看