...
def process_single_file(input_path, output_path,DURATION=None,start_time_offset = 0,end_time_offset = 0):
    """
    处理单个Excel文件
    ...
    if 'trans_point' in excel_data and not excel_data['trans_point'].empty:
        ...
        if 'timestamp' in trans_point_df.columns and len(trans_point_df) > 0:
            start_time = trans_point_df['timestamp'].iloc[0] + start_time_offset
            print(f"  起始时间 (trans_point第一个时间戳): {start_time}")
    if DURATION is not None:
        end_time = start_time + DURATION + end_time_offset
    else:
        ...
        if 'alpha' in excel_data and not excel_data['alpha'].empty:
            ...
            if 'alpha' in alpha_df.columns and 'timestamp' in alpha_df.columns:
                ...
                if not alpha_1_rows.empty:
                    end_time = alpha_1_rows['timestamp'].iloc[0] + end_time_offset
...
def main():
    ...
    up_dir = "/home/xzh2/ros1/gpir_Modify/rosrecord2/"
    input_dir = up_dir+"/"+args.Exp_setting+"-"+args.method_setting+"/result-mid"
    output_dir = up_dir+"/"+args.Exp_setting+"-"+args.method_setting+"/result-for-plot-traj"
    ...
    for xlsx_file in xlsx_files:
        ...
        if 'Compare1' ==  args.method_setting:
            ...
            process_single_file(str(xlsx_file), str(output_file),duration,start_time_offset = start_time_offset,end_time_offset = end_time_offset)
        else:
            process_single_file(str(xlsx_file), str(output_file),start_time_offset = start_time_offset,end_time_offset = end_time_offset)
        print()
