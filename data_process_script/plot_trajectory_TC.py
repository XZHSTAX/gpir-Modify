...
def draw_lanes(lane_files_directory):
    ...
    for i in range(1, 5):
        ...
        if os.path.exists(file_path):
            ...
            plt.plot(-centerline_points[:, 1],centerline_points[:, 0],  '--', color='gray', alpha=0.5, linewidth=1)
            plt.plot(-left_boundary[:, 1],left_boundary[:, 0],  color='black', linewidth=3)
            plt.plot(-right_boundary[:, 1],right_boundary[:, 0],  color='black', linewidth=3)
        else:
...
# 获得分段数据
def get_segments_data(xls,df_ego):
    trans_point = xls.parse('trans_point')
    trans_point_time = trans_point['timestamp'][0]

    # auto_mask = (df_ego['timestamp'] >= 0) & (df_ego['timestamp'] <= trans_point_time)

    auto_mask = df_ego['timestamp'] <= trans_point_time
...
def plot_trajectories_from_file(file_path, output_dir):
    ...
    for sheet_name in vehicle_sheets:
        ...
        if 'x' in df.columns and 'y' in df.columns:
            plt.plot(df['y'].to_numpy(), df['x'].to_numpy(), alpha=0.8, linewidth=5, linestyle='--')
        ...
        if not df_ego.empty and df_ego.columns[0].lower() in ['time', 'timestamp']:
            ...
            for _, row in df_ego.iterrows():
                if row['relative_time'] >= last_annotated_time + 0.5:
                    plt.plot(row['y'], row['x'], 'o', 
                            color='gray',         # 线条颜色
                            markerfacecolor='gray',  # 点的填充色
                            markeredgecolor='w',   # 点的边框色
                            markeredgewidth=1.5,       # 点的边框宽度
                            markersize=14)               # 线宽
                    ...
                    plt.text(row['y'], row['x'] + 0.5, fr'$t_{{{nt}}}$', fontsize=20, ha='center', va='bottom')
    ...
    font_prop = fm.FontProperties(fname='/usr/share/fonts/truetype/simsun.ttc', size=16)
    ...
    output_filename = os.path.join(output_dir, f"{os.path.splitext(os.path.basename(file_path))[0]}.pdf")
